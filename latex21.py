...
def onAdvanced(self):
    ...
    # must be run after setupUi since everything is defined in setupUi ...
    frm.verticalLayout.addWidget(apply_to_all)
    # mod end
    ...
    if apply_to_all.isChecked():
        if askUser("Write these values for the header/footer to ALL notetypes?"):
            changeLaTeX(self.model['latexsvg'],
                        self.model['latexPre'], self.model['latexPost'])
